...
## Definiendo los Roles de la Red
...
* Hacer un compromiso firmando los datos del bloque que proporcionan utilizando el [esquema de firma BLS](https://en.wikipedia.org/wiki/BLS_digital_signature) garantizando su usabilidad. Estas firmas son verificadas por un contrato EigenDA en Ethereum

<Info>
  Aprenda más en la sección de [Disponibilidad de Datos](/es/network/network/introduction/concepts/data-availability).
</Info>

El DA de Mantle impulsado por la tecnología EigenDA consiste en un conjunto de interacciones soportadas entre cuatro componentes principales como se describe a continuación.
...
* Forzar sin permiso la resolución en la cadena de operaciones problemáticas, resultando en la pérdida de la apuesta de un operador
...
* Verificar la validez de los datos enviados por los Desafiantes contra un compromiso firmado, y aplicar el recorte según lo dictado por el protocolo de prueba de custodia
...
El diagrama a continuación ilustra cómo el Secuenciador y el Verificador de Rollup encajan juntos. Tenga en cuenta que las acciones en rojo son realizadas por un**Secuenciador**, mientras que las acciones en azul son realizadas por un**Verificador de Rollup**:
