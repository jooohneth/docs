---
- title: Развертывание узла Rollup Verifier/Replica
- description: Разверните узел верификатора и синхронизируйте данные роллапа как
    часть сети Mantle
---
...
* **RAM** - мин. 16 ГБ
* **Хранилище** - мин. 100 ГБ свободного места на диске (HDD подойдет на данный момент, SSD лучше)
...
Мы рекомендуем запускать узел с последним [snapshot](https://github.com/mantlenetworkio/networks/blob/main/run-node-mainnet.md#download-latest-snapshot-from-mantle), чтобы вам не пришлось долго ждать синхронизации данных.
...
<Tabs>
  ...
  <Tab title="Запуск узла из бинарного файла">
    ...
    <Info>
      ...
      3. Если скорость синхронизации низкая, попробуйте изменить ваш DTL URL, подробнее можно узнать [здесь](/ru/network/network/for-validators/deploy-a-node/deploying-a-rollup-verifier-replica-node#network-dtl-url).
    </Info>
